...
pub struct State<'a, #[cfg(feature = "gui")] EventHandler: gui::event::GuiEventHandler> {
    ...
    rotation_speed: f32,
    #[cfg(feature = "gui")]
    gui_state: gui::state::EguiState<EventHandler>,
}
...
impl_State! {
    impl State {
        ...
        pub async fn new(
            ...
            #[cfg(feature = "gui")]
            let gui_state = gui::state::EguiState::new(&device, &config, event_handler, model_loader);
        ...
        fn handle_gui_events(&mut self) {
            use gui::GuiAction;
            use crate::renderer::animation::AnimationState;
